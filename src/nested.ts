...
export function sumPoints(questions: Question[]): number {
    const nums = questions.map((question) => question.points);
    ...
    return questions.map((question) => {
        if (question.id === targetId) {
            ...
            if (targetOptionIndex === -1) {
                modifiedQuestion.options = [...question.options, newOption];
            } else if (
                targetOptionIndex >= 0 &&
                targetOptionIndex < question.options.length
            ) {
                modifiedQuestion.options = question.options.map(
                    (option, index) =>
...
export function duplicateQuestionInArray(
    ...
    const duplicatedIndex = questions.findIndex(
    ...
    if (duplicatedIndex !== -1) {
        const duplicatedQuestion = duplicateQuestion(
            newId,
            questions[duplicatedIndex]
        );
        return [
            ...questions.slice(0, duplicatedIndex + 1),
            duplicatedQuestion,
            ...questions.slice(duplicatedIndex + 1)
        ];
    }
    return questions;
}
